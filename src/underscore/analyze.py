...
Nothing about the video leaves the machine unless you choose a cloud backend
(--llm cli or api), and even then only the transcript text and cut list are sent.
"""
...
def scene_cuts(video: str | Path, threshold: float = 27.0) -> list[float]:
    try:
        ...
        raise RuntimeError("pip install 'rawlslab-underscore[video]' for scene detection") from e
