...
| `local` seam (local model runner) | partial | `UNDERSCORE_LOCAL="<runner command>" underscore compose brief.json --llm local`; selection tested by `test_14` | code path; no recorded run |
...
| Compose without a configured backend refuses | partial | `underscore compose brief.json` with no `UNDERSCORE_CLI` | refuses with the right message, but as a Python traceback (RuntimeError), not a one-line error and exit 2 |
...
| Stems from the Sonic Pi engine (one solo pass per layer) | missing | roadmap line; `render.py` returns `stems: {}` for Sonic Pi | none; catalog beds have no stems; v0.2 item on `next` per decision 15 |
...
| `--ship-anyway` (write a failed bed as shippable) | partial | `underscore score --help` lists it; `cli.py` honors it | no test exercises the override |
...
| Transcript model download on first run is documented | partial | README does not say faster-whisper fetches the `base` model on first use | doc line to add |
...
| Release layout: six collection zips, previews zip, catalog.json, checksums, every WAV, `.rb`, and preview MP3 as an individual asset | shipped | `gh release view catalog-2026.09 --json assets --jq '.assets|length'` (334) | `release-SHA256SUMS.txt` covers every file |
...
| CC0 dedication embedded in the audio file itself (ID3 or BWF tag with bed id and manifest hash) | missing | launch gap register item 4; no `mutagen` or tag writer in `export.py` | none |
...
| Rot self-check configured in this repo (`rot.yaml`, weekly workflow) | missing | flip check: "rot not on PATH or no rot.yaml here" | the Rot lane runs it from outside; a `rot.yaml` here would let CI run it |
...
| 20-second demo video or cast | missing | checklist E asks for "a screenshot or a 20 s demo"; the screenshot is what exists | none |
...
| Shared core adoption (`brief`, `gates` from rawlslab-core) | missing | `~/core/ADOPTION.md` Underscore section; no `src/underscore/_vendor` | Underscore is the one family tool not vendoring core; optional before freeze |
...
- The README Requirements section should say faster-whisper downloads its `base` model on first use (POLICY.md's "nothing is sent" claim is about the compose step and stays true).
