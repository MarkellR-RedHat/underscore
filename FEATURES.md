...
| Stems from the Sonic Pi engine (one solo pass per layer) | shipped on `next` (v0.2), rendered for the glass collection on the release | on `next`: `underscore score --brief b.json --program p.rb --engine sonicpi --stems`; `scripts/catalog_stems.py --collection glass`; `python -m pytest -q tests/test_stems.py` | 72 glass stem WAVs on catalog-2026.09 (pads, lead, bass, drums per bed; a synth shared by pad and bass is split by pitch), tagged and in the sums; stems are click-aligned and trimmed to within a few ms of the mix, same notes at the same times, not sample-identical (synth phases differ per boot) |
...
| Loop-safe variants of the 108 catalog beds | missing | decision 4: nice-to-have; Markell's call (question 19); and decision 27 now plans a replacement catalog (`docs/CATALOG-2.md` on `next`) | none |
...
| RC rehearsal: the whole public flip, in scratch, without touching the repository or GitHub | shipped | `bash scripts/rc-rehearsal.sh` (calls the family's shared `~/launch/comms/flip/public-branch.sh` for the orphan branch and its flip check, then builds the wheel from that clean tree, installs it into a clean virtualenv outside any checkout, and runs `init` and `score --engine synth` from the installed wheel with every backend variable unset) | **PASS at b6e3f0e on 2 September 2026**: 11 checks, 0 failures. One commit on the public branch, INTERNAL.md gone, flip-check PASS with 5 warnings, wheel `rawlslab_underscore-0.1.0rc1`, gate passed at -14.2 LUFS with 8 files, `gates.json` and the CC0 text in the bundle |
